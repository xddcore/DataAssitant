...
Classes_image_num = 100 #每个分类101[0-100]张图片|
FPS = 30 #33帧左右(1000/30 = 33)(若不计保存耗时)
##################################################################################
bg = lcd.RED
text = lcd.WHITE
boot_press_flag = 1
...
def boot_key_irq(pin_num):#boot按钮中断
    ...
    if(boot_press_flag == 1):
        ...
        if(time_diff >120 and time_diff <500):
            ...
            ui_num = 1
            if(ui_num == 1 and first_button == 1):#已进入拍摄
                while(image_num <= Classes_image_num):
                    lcd.display(sensor.snapshot(),oft=(0,0))
                    #shoot_flag = 1
                    image_data.save("/sd/image/"+str(claass)+"/"+str(image_num)+".jpg", quality=95)
                    lcd.draw_string(160, 224,"ok!"+str(image_num))
                    image_num = image_num + 1
                    utime.sleep_ms(FPS)
                lcd.draw_string(50, 224,"Has been successfully collected!")
                utime.sleep_ms(2000)
...
def draw_help_ui():#显示帮助页面
    lcd.clear(bg)
    lcd.draw_string(20, 10, "Data Collection Assistant (Detection)", text, bg)
