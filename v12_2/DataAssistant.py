...
def process_image_classification(image,mwidth=image_weight, mheight=image_hight):#目标分类用
    ...
    '''
    return new_im
def process_image_Object_Detection(image,mwidth=224, mheight=224):#目标检测用,yolov2下采样/32
    w,h = image.size
    #print(w,h,mwidth,mheight)
    if w<=mwidth and h<=mheight:#小于处理尺寸
        #print(filename,'is OK.')
    ...
    if(video_handel_flag == 0):#图片处理
        try :
            for class_name in classes:#class_name:文件夹标签名
                ...
                if((not len(glob.glob(class_path+'0.jpg'))) and (not len(glob.glob(class_path+'0.png'))) and (mode_status == 2)):#检查是否有0.jpg/0.png文件
                    if(language == "chinese"):
            ...
            broken_flag = 1#文件破损
            if(language == "chinese"):
                log_text.AppendText("\n"+img_path+"|图片文件破损，软件无法处理，请手动删除破损图片文件")
            elif(language == "english"):
                log_text.AppendText("\n"+img_path+"|The image file is damaged and the software cannot process it, please delete the damaged image file manually.")  
        if(broken_flag == 0):
            if(language == "chinese"):
                log_text.AppendText("\n\n\n处理成功！")
            elif(language == "english"):
